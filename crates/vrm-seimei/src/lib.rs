...
impl VrmAvatar {
    ...
    pub fn load(bytes: &[u8]) -> Result<VrmAvatar, String> {
        ...
        // カメラ方位/歩行/各種ジオメトリの「VRM0 だけ符号反転」特殊分岐が一切不要になる。
        if src_is_vrm0 {
            for i in 0..nn {
                let t = nodes_t[i];
...
/// its rest-shape value, so a morphed bump (a small protrusion or enlarged region) is shaded
...
fn apply_morphs(base: &SkinMesh, deltas: &[Vec<[f32; 3]>], active: &[(usize, f32)]) -> SkinMesh {
    ...
    if moved.iter().any(|&m| m) {
        ...
        // 法線スムージング: 低ポリ部位(小さな突起等)の幾何法線は面ファセットが粗く、また
